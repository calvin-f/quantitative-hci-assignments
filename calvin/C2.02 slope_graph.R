...
data_knobology_within %>% 
  ...
  ggplot(aes(x = device, y = time, group = interaction(participant, vision))) +  # TODO: add the `group` parameter
	  geom_point() +
	  geom_line() +
    geom_text(aes(label = label), nudge_x = -0.03)                               # TODO: replace NULL with a geom_text() call
