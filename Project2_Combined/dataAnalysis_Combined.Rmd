...
First off, we define a helper function `readLog` which takes a file path and parses the filename and data of the file.
...
In the QQ-Plot, we find a slight deviation from the QQ-Band at around 25. However, because the sample size is very large, we can also see, that the QQ-Band is rather narrow.
...
If we have a look at the histogram of the residuals, we can see the slight skewness at about 25 as well. Overall, the distribution of residuals is still very close to a normal distribution. 
```{r}
...
After exploring and analyzing our data, we have not seen a significant effect on word per minute for different typing styles. When looking at condition and typing style, we also don't find a significant effect on WPM. Still, it has to be noted that the condition itself shows a significant effect on the resulting WPM.
