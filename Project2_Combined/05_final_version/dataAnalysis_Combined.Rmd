...
This project is based on the paper *[How We Type: Movement Strategies and Performance in Everyday Typing](https://dl.acm.org/doi/10.1145/2858036.2858233)* by Anna Maria Feit, Daryl Weir, and Antti Oulasvirta (2006).
We used the data set the authors have provided within the course of their study.
The aim of the data analysis will be to answer the following to two research questions (RQ):
...
After exploring and analyzing our data, we have not seen a significant effect on word per minute for different *typing styles* (RQ1). 
Moreover, after looking at *condition* * *typing style*, a significant difference could not be found, which was confirmed by the model (RQ2). Still, it has to be noted that the *condition* itself shows a significant effect on the resulting *wpm.* Both findings (RQ1, RQ2) were not suprising after looking at the descriptive statistics and visualizations.
