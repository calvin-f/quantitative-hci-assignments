...
the_answer <- 
  mc_input %>%
  mutate(age_group = ifelse(between(age,0,2),"Baby",
         ifelse(between(age,3,39),"Young adults",
         ifelse(between(age,40,59),"Middle-aged adults",
         "Old adults"
         )))) %>%
  mutate(score_level = case_when(score < 6 ~ "low", score > 5 ~ "high"))
