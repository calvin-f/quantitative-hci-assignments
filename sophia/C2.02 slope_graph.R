...
data_knobology_within %>%
  ...
  mutate(participant = ifelse(device == "Touch", "", data_knobology_within$participant)) %>%                                    # TODO: add a step to replace the label on the `Touch` side with "" to hide them
  ggplot(aes(x = device, y = time, group = interaction(participant2,vision))) +  # TODO: add the `group` parameter
	  geom_point() +
	  geom_line() +
    geom_text(aes(label = participant), nudge_x = -0.07)                               # TODO: replace NULL with a geom_text() call
